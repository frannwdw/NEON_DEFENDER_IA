...
/**
 * Inicializa la webcam, micrófono y redes neurales asíncronas
 */
async function iniciarJuego() {
    estadoApp = "cargando";

    // LEDs en parpadeo táctico
    ...
    if (textoEstadoVision) textoEstadoVision.innerText = "Preparando...";

    dibujarPantallaCarga("Cargando reactores neurales...");

    try {
        // A. CONEXIÓN DE LA WEBCAM
        dibujarPantallaCarga("Enlazando entrada de webcam...");
        camaraWeb = new tmPose.Webcam(640, 360, true);
        await camaraWeb.setup();
        await camaraWeb.play();
        
        if (textoEstadoPostura) textoEstadoPostura.innerText = "Cámara Activa";
        if (ledPostura) ledPostura.className = "status-led ready";

        // Inyectar lienzo webcam al mini-visor PIP
        ...
        // B. CONEXIÓN DE MICRÓFONO
        dibujarPantallaCarga("Sintonizando receptor acústico...");
        try {
            await navigator.mediaDevices.getUserMedia({ audio: true });
        } catch (e) {
            console.warn("Acceso a micrófono restringido:", e.message);
        }

        // C. DESCARGA MODELO DE POSTURA
        dibujarPantallaCarga("Descargando pesos neuronales PoseNet...");
        ...
        // D. DESCARGA MODELO DE AUDIO
        dibujarPantallaCarga("Estabilizando frecuencia de comandos...");
        try {
            ...
            // Iniciar receptor continuo libre de latencia
            modeloVoz.listen(resultado => {
                ...
                // 💥 DETECCION DISPARO CONTINUO: Class 2, pum, pew, etc.
                if (maxP >= 0.70 && (
            ...
            console.error("Fallo de inicialización de micrófono:", eAudio);
        ...
        // Arranca el bucle de esqueleto asíncrono
        bucleIA();
        
        dibujarPantallaListo();

    } catch (err) {
        console.error("Error en enlace multimedia táctico:", err);
        dibujarPantallaCarga("ERROR DE SISTEMA: " + err.message);
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (btnIngresar) {
        btnIngresar.addEventListener("click", () => {
            // Ocultar landing de bienvenida e ingresar al radar táctico
            if (landingPage && consoleContainer) {
            ...
            // Iniciar webcam y modelos neuronales
            iniciarJuego();
        });
    }
